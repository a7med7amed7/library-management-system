...
class ReportingService {
  async generateReport({
    ...
    if (format.toLowerCase() === 'csv') {
      return await this.generateCSV(reportData, report_type);
    } else {
  ...
  getMostBorrowedBook(data) {
    ...
    return (
      Object.entries(bookCounts).sort(([, a], [, b]) => b - a)[0]?.[0] || "None"
    );
  }
  ...
  getMostActiveBorrower(data) {
    ...
    return (
      Object.entries(borrowerCounts).sort(([, a], [, b]) => b - a)[0]?.[0] ||
      "None"
    );
  }
  ...
  async generateExcel(data, reportType) {
    ...
    const wsData = [
      ...
      ...formattedData.map((row) =>
        headers.map(
          (header) => row[header.toLowerCase().replace(/ /g, "_")] || ""
        )
      ),
    ];
  ...
  formatReportData(data, reportType) {
    ...
    return data.map((item) => {
      switch (reportType) {
        ...
        case "last_month_borrowing":
          return this.formatReportData(data, "borrowing");
        case "last_month_overdue":
          return this.formatReportData(data, "overdue");
        default:
          return item;
      }
    });
  ...
  async generateCSV(data, reportType) {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Add analytics as comments at the end if available
        ...
        if (data.analytics) {
          ...
          Object.entries(data.analytics).forEach(([key, value]) => {
            ...
            finalCsvString += `# ${metricName}: ${value}\n`;
          });
        }

        resolve(Buffer.from(finalCsvString, 'utf8'));
      } catch (error) {
        reject(new appError(`Error generating CSV: ${error.message}`, 500));
      }
    });
  }

  // Method to export overdue borrows for last month
  async exportLastMonthOverdue(format = 'xlsx') {
    const lastMonthStart = startOfMonth(subMonths(new Date(), 1));
    const lastMonthEnd = endOfMonth(subMonths(new Date(), 1));
  ...
  // Method to export all borrowing processes for last month
  async exportLastMonthBorrowing(format = 'xlsx') {
    const lastMonthStart = startOfMonth(subMonths(new Date(), 1));
    const lastMonthEnd = endOfMonth(subMonths(new Date(), 1));
  ...
  // Enhanced analytics for specific periods
  async generatePeriodAnalytics(startDate, endDate, borrowerId = null) {
    const whereClause = {
      checkout_date: {
    ...
    if (borrowerId) {
      whereClause.borrower_id = borrowerId;
    }

    const borrowingData = await BorrowingHistory.findAll({
      where: whereClause,
      include: [
        {
          model: Book,
    ...
    const analytics = {
      ...
      borrowing_stats: {
        total_borrowings: borrowingData.length,
        unique_borrowers: new Set(borrowingData.map(item => item.Borrower.id)).size,
        unique_books: new Set(borrowingData.map(item => item.Book.id)).size,
        daily_average: (borrowingData.length / Math.ceil((new Date(endDate) - new Date(startDate)) / (1000 * 60 * 60 * 24))).toFixed(2)
      },
      return_stats: {
        returned_books: borrowingData.filter(item => item.is_returned).length,
        pending_returns: borrowingData.filter(item => !item.is_returned).length,
        return_rate: borrowingData.length > 0 ? 
          ((borrowingData.filter(item => item.is_returned).length / borrowingData.length) * 100).toFixed(2) + '%' : '0%'
      },
      popular_books: this.getTopBooks(borrowingData, 5),
      active_borrowers: this.getTopBorrowers(borrowingData, 5),
      overdue_analysis: await this.getOverdueAnalysisForPeriod(startDate, endDate, borrowerId)
    };

    return analytics;
  }

  getTopBooks(data, limit = 5) {
    const bookCounts = {};
    data.forEach((item) => {
      const bookKey = `${item.Book.title} by ${item.Book.author}`;
  ...
  getTopBorrowers(data, limit = 5) {
    const borrowerCounts = {};
    data.forEach((item) => {
      borrowerCounts[item.Borrower.name] = (borrowerCounts[item.Borrower.name] || 0) + 1;
    });
  ...
  async getOverdueAnalysisForPeriod(startDate, endDate, borrowerId = null) {
    const whereClause = {
      checkout_date: {
        [Op.between]: [startDate, endDate],
      },
    ...
    const overdueBooks = await BorrowingHistory.findAll({
      where: whereClause,
      attributes: [
        [
          sequelize.fn(
    ...
    return {
      ...
      overdue_breakdown: {
        ...
        one_to_two_weeks: overdueDays.filter((days) => days > 7 && days <= 14).length,
